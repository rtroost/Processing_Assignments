...
void draw() {
	...
	/* Draw the heatmap */
	if(gfx_options.getArrayValue()[0] != 0.0)
		drawHeatmap(105, 100, 680, 475);

	/* Draw the wind directions map */
	strokeWeight(1);
	if(gfx_options.getArrayValue()[1] != 0.0)
...
void controlEvent(ControlEvent theEvent) {
  ...
  if (theEvent.isGroup()) {
		...
		if(theEvent.isFrom(cp5.getController("btn_go"))) {
			if(cp5.getGroup("ddl_day").getValue() != 0.0 &&
				cp5.getGroup("ddl_month").getValue() != 0.0 &&
				cp5.getGroup("ddl_year").getValue() != 0.0) {
...
void drawWindDirections(int startX, int startY, int endX, int endY){
